...
import useLocalStorage,{useSessionStorage} from '../../hooks/local-storage-hook';
import * as Styled from '../../styledJS/common/common.styled';
import {GridDiv} from './Styled.js';
...
const searchForm = React.memo((props) => {
    ...
    const [autoExceptionForm,setAutoForm]=useState({      
    });
    ...
    const [advancedForm,setAdvancedSearchForm]=useState({});   
    const [dropdownTypes,setDropdownStore]=useState({actionTypes:[],statuses:[]});
   
    const dispatch = useDispatch();
    const exceptionList = useSelector(state => state.masterExceptions.exceptions);
    const clientDetailsList = useSelector(state => state.masterExceptions.clientDetails);
    ...
    const saveSettings=(event)=>{
     if(event.item==='Save Criteria' && Object.keys(autoExceptionForm).length>0){
    ...
    const inputChangedHandler = (obj,type) => {
      ...
      setAutoForm(updatedFormElement);
    ...
    },[autoExceptionForm, advancedForm, onFetchExceptions, setSessionStoredForm, setSessionStoredAdvancedForm]);
    ...
    const resetAttributeForAdvSearch =useCallback((attribute)=>{
    ...
    useEffect(()=>{
      ...
      if(queryParams!==undefined && queryParams.backFlag){
        ...
        setAutoForm(sessionStoredForm);
      
      }
      else if( Object.keys(searchForm).length>0){
        setAutoForm(searchForm);
        //onFetchExceptions(searchForm);
      }
     
    },[onFetchExceptions, props.location, searchForm, sessionStoredForm]);
    ...
    useEffect(()=>{
     ...
     if(loadingFlag){
      grid=<div style={{width:"100%",textAlign: "center"}}>
     <Loader
         type="Circles"
         color="#00BFFF"
         height={100}
...
grid=<p>Exception thrown!!!</p>
      ...
      {...props}
       ...
       dropdownTypes={dropdownTypes}
        exceptionList={exceptionList.map(dataItem => Object.assign({ selected: false }, dataItem))}></ExceptionsGrid>
    }
    ...
    chips =useMemo(()=>{
      ...
      if(advancedSearchApplied){
        ...
        return(
        <div ><span style={{display:"block",color:"white",fontWeight:"bold",marginLeft:"5px",paddingLeft:"10px",borderBottom:"1px solid white"}}>Advanced Search Fields </span>
        <Chips data={advancedForm}
         resetValForAttribute={(attribute)=>resetAttributeForAdvSearch(attribute)}/></div>);
      }
...
<div className="col-xs-12 col-sm-12 col-md-12 col-lg-12">
  ...
  <Styled.PageTitle>   
    ...
    <ToolbarSeparator />
    <ToolbarItem >
    <a href="#" title="Search" onClick={showAdvancedSearch}   data-toggle="modal" data-target="#exampleModalCenter"> <i className="fa fa-search"></i>&nbsp;Advanced Search</a>
...
<CustomAutoSuggest value ={autoExceptionForm.clientName || ''} id="clientName" name="clientName" onChange={inputChangedHandler} type="ClientName" />
...
<CustomAutoSuggest id="clientGroup" name="clientGroup" value={autoExceptionForm.clientGroup || ''} size="lg" onChange={inputChangedHandler} type="ClientGroup"  />
...
<Dropdown
            ...
            onChangeHandler={event=>inputChangedHandler(event.target,'string')}
            />
          </div>
...
<DatePicker
 name="jobDateFrom"  value={autoExceptionForm.jobDateFrom || ''} size="md" format="dd/MMM/yyyy" onChange={event=>inputChangedHandler(event.target,'date')}  />
       &nbsp;     
       </Styled.StyledDatePicker>
       <Styled.StyledDatePicker>
<DatePicker name="jobDateTo"  value={autoExceptionForm.jobDateTo || ''} size="md" format="dd/MMM/yyyy" onChange={event=>inputChangedHandler(event.target,'date')} />
</Styled.StyledDatePicker>
&nbsp;
<Styled.TextField id="jobDays" name="jobDays" value={autoExceptionForm.jobDays || ''} size="sm" onChange={event=>inputChangedHandler(event.target,'')}  />
              </div>
</div>
<div className="col col-sm-12 col-md-4 col-lg">
<Styled.FieldLabel >AWB</Styled.FieldLabel>
<CustomAutoSuggest id="shpNo" name="shpNo" value={autoExceptionForm.shpNo || ''} size="lg" onChange={inputChangedHandler} type="Awb"  />
</div>
...
<button className="btn btn-primary" disabled={loadingFlag} onClick={()=>fetchExceptions()}>Search</button>&nbsp;&nbsp;
<button className="btn btn-primary"  onClick={()=>onFetchClientDetails()}>Client Details</button>&nbsp;&nbsp;
...
{advancedSearchFlag && <AdvancedSearchDialog  dropdownTypes={dropdownTypes} advancedSearchFields={advancedForm} applySearch={applyAdvancedSearch} 
